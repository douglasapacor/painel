...
> = async () => {
  try {
    const desc = await fetchApi.get(legacy.boletim.description(12))
    const state = await fetchApi.get(legacy.classificadores.stateInfo("RS"))
...
const ClassifiersRS: NextPage<classifiersRS> = props => {
  return (
    <SiteFrame>
      <Grid container>
        <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
          <section
            style={{
              ...
              height: "250px",
              fontSize: "1.4rem",
              backgroundImage: `url('/images/textura_banner.png'), url('/images/header_areas/banner_rs.jpg')`,
              ...
              {props.list &&
                props.list.map((i, index) => (
                  ...
                  >
                    <a
                      className="be-main-a"
                      href={`/site/classificadores/RS/${i.id}/ler`}
                    >
